...
for s=1:numSampControl
    ...
    xPatients =  gamrnd(mu, sigma, [numPatientSets, numSampPatients]);
    xPatients = xPatients - repmat(median(xPatients,1),200,1); % center on median rather than mean
    
    for i=1:numDeltas
        fprintf('running Control N=%g Delta = %g\n',SampControl(s),delta(i));
        xTest = xPatients + median(xControl) + delta(i);
        ...
        % the null hypothesis is that percentilles are identical
        % we can thus permute any percentilles and we should get the same
        % results as the classification observed if there is no effect       
        tmp2 = NaN(numPatientSets,numSampPatients);
        H0_nb_abnormal_patients = NaN(numSampPatients,600);
        out2 = prctile(xTest,[0:100]);        
        for perm= 1:1000
