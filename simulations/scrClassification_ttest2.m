...
%Sample control set
xControl = normrnd(mu,sigma, [1, 10000]);

for s=1:numSampControl
    xControl = xControl(randi(1000,SampControl(s),1)); % pick SampControl(s) controls
    
    out = prctile(xControl,[0:100]);
    ci = [out(3) out(98)];
    
    nb_abnormal_controls_ml(s) = (sum(xControl<ci(1))+sum(xControl>ci(2))) / SampControl(s) * 100;
    ...
    for i=1:numDeltas
        ...
        for b= 1:600
            ...
            H0_nb_abnormal_patients(:,b) = sum(tmp2<ci(1),1)+sum(tmp2>ci(2),1);
        end
        ...
        resultsClassif_ml(s,i) = 100*sum(h_ml)/numSampPatients;
    ...
    classification_rate_ml(s,:,:) = reshape(cell2mat(nb_abnormal_patients),1000,size(nb_abnormal_patients_ci,2));
end
